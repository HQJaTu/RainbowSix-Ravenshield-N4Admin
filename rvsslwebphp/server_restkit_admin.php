...
if (!isset($_COOKIE["RVSsessionip"]) or !isset($_COOKIE["RVSsessionport"])) {
    die ("cookie for ip+port error!");
}
    ...
    if ($sendit == True and isset($packetstring)) {
        ...
        if (is_resource($fp)) {
            ...
            if ($bytes_left['unread_bytes'] > 0) {
                $respond .= fread($fp, $bytes_left['unread_bytes']);
            }
            fclose($fp);
        }
    }
...
<center>
    <table border=0 cellspacing=0 width="<?php print $awidth; ?>">
        <form name="listadm" action="server_restkit_admin.php">
            <tr>
                <td align=left class=oben background="images/<?php print $design[$dset]; ?>_header.gif">
                    <?php print ShowFlags("server_restkit_admin.php", ""); ?>Admin Server-Restriction-Kits / <?php print $ip; ?>
                    :<?php print $port; ?>[SrBnPt]
                </td>
                <td align=right class=oben background="images/<?php print $design[$dset]; ?>_header.gif"><img
                        src="images/clear.gif" width=10 height=10 name="back"><a class=nav2 href="server_admin.php"
                                                                                 onMouseOver="mi('back')"
                                                                                 onMouseOut="mo('back')"><?php print $text_back; ?>
                        &nbsp;</a></td>
            </tr>
            <tr>
                <td colspan=2>
    ...
    <table border=0 cellspacing=0 class=frame width="<?php print $swidth; ?>">
        <tr>
            <td background="images/<?php print $design[$dset]; ?>_header.gif" class=bigheader align=center>FeedbackBox</td>
        </tr>
        <tr>
    ...
    <table border=0 cellspacing=0 width="<?php print $awidth; ?>">
        <tr>
            <td align=center class=oben background="images/<?php print $design[$dset]; ?>_header.gif"
                width="50%"><?php print $text_restkit_tabletitleadd; ?> (Click to add)
            </td>
            <td align=center class=oben
                background="images/<?php print $design[$dset]; ?>_header.gif"><?php print $text_restkit_tabletitle; ?> (Click to remove)
            </td>
        </tr>
    </table>
    ...
    if (is_resource($fp)) {
        ...
        if (is_resource($fp2)) {
            ...
            if ($bytes_left['unread_bytes'] > 0) {
                $serverrepeatadd .= fread($fp2, $bytes_left['unread_bytes']);
                $failed = False;
            }

            $numbercheckadd = explode(" �", $serverrepeatadd);
            foreach ($numbercheckadd as $item) {
                $numbercheckarrayadd[substr($item, 0, 2)] = substr($item, 3, strlen($item) - 3);
            }
        ...
        if (strstr($serverrepeat, "/")) {
            if (strstr($serverrepeatadd, "/")) {
                ...
                for ($i = 0; $i < 10; $i++) {
                    ...
                    if ($allrest) {
                        echo "<b><img src=\"images/clear.gif\" width=10 height=10 name=\"resta" . $hitx . "1\"><a class=nav href=\"server_restkit_admin.php?Submit=changerestkit&restdata=" . $allrest . "\" onMouseOver=\"mi('resta" . $hitx . "1')\" onMouseOut=\"mo('resta" . $hitx . "1')\">" . $text_all . "</a>";
                    } else {
                        echo "&nbsp;";
                    }
                    ...
                    $hitx++;
                    if ($allrested) {
                        echo "<b><img src=\"images/clear.gif\" width=10 height=10 name=\"resta" . $hitx . "2\"><a class=nav href=\"server_restkit_admin.php?Submit=changerestkit&restdata=" . $allrested . "\" onMouseOver=\"mi('resta" . $hitx . "2')\" onMouseOut=\"mo('resta" . $hitx . "2')\">" . $text_all . "</a>";
                    } else {
                        echo "&nbsp;";
                    }
                    $hitx++;
    ...
    ?>
    <br><font class="normal"><a class=nav href="server_admin.php"><?php print $text_back; ?></a>
