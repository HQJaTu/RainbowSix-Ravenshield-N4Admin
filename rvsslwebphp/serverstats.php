...
<script language="javascript">
    ...
    function mim(n)
    {
        document["mappic"].src = n;
    }
    function mom(n)
    {
        document["mappic"].src = "images/defaultmap.jpg";
    }
    // -->
</script>
<?php
...
if (isset($Statstable[$mode])) {
    ...
    $resstats = db_query($db, "SELECT " . $Statstable[$mode] . ".map as map, sum(" . $Statstable[$mode] . ".kills) as sumkills, sum(" . $Statstable[$mode] . ".deaths) as sumdeaths ,sum(" . $Statstable[$mode] . ".fired) as sumfired,sum(" . $Statstable[$mode] . ".hits) as sumhits  FROM " . $Statstable[$mode] . " INNER JOIN " . $Playertable . " ON (" . $Statstable[$mode] . ".fromid = " . $Playertable . ".id) WHERE " . $Playertable . ".serverident like '" . $serverident . "' and " . $Playertable . ".ubiname<>'' GROUP by " . $Statstable[$mode] . ".map ORDER BY " . $Statstable[$mode] . ".map ");
    $sumkills = 0;
    $sumdeaths = 0;
    $sumfired = 0;
    $sumhits = 0;
    ...
    while ($stats = db_fetch_array_assoc($resstats)) {
        ...
        if (!file_exists($mapimage)) {
            $mapimage = "images/defaultmap.jpg";
        }
