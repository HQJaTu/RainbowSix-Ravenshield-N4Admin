...
if (!isset($_COOKIE[$cookiename])) {
    die("error, no pw for this server in cookie!");
}
...
if (isset($delban)) {
    $send = "ADMIN " . $pw . " DELBAN " . $delban;
}
if (isset($addban)) {
    $send = "ADMIN " . $pw . " ADDBAN " . $addban;
}
if (!isset($sendit)) {
    $sendit = "nosend";
}
...
<center>
    <table border=0 cellspacing=0 width="<?php print $awidth; ?>">
        <form name="listadm" action="server_banlist_admin.php">
            <tr>
                <td align=left class=oben background="images/<?php print $design[$dset]; ?>_header.gif">
                    <?php print ShowFlags("server_banlist_admin.php", ""); ?>Admin Server-BanList / <?php print $ip; ?>:<?php print $port; ?>
                    [SrBnPt]
                </td>
    ...
    <table border=0 cellspacing=0 class=frame width="<?php print $awidth; ?>">
        <tr>
        ...
        <tr>
            <td class=randende colspan=2 align=center><?php print $mess; ?></td>
        </tr>
        <tr>
            <td background="images/<?php print $design[$dset]; ?>_header.gif" colspan=2 class=bigheader align=center>Add Ban</td>
        </tr>
        <tr>
            <td class=randende colspan=2 align=center>
                <input type="text" class="textfield" name="tobanid" size="60" maxlength="32" value="BanID"
                       title="Enter BanID to ban here">
                &nbsp;<input type="submit" class="button" name="Submit" value="AddBanID"><input type="hidden"
                                                                                                class="button"
                                                                                                name="Submit"
                                                                                                value="AddBanID"></td>
        </tr>
        <tr>
            ...
            <td align=center class=oben background="images/<?php print $design[$dset]; ?>_header.gif">Banned-ID (Click to
                remove)
            </td>
        </tr>
        <?php
    ...
    <center><br><font class="normal"><a class=nav href="server_admin.php"><?php print $text_back; ?></a>
