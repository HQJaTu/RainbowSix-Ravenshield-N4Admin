...
if (isset($_GET['server'])) {
    if ($_GET['server'] == "All") {
        ...
        $serverident = $servernameident[$_GET['server']];
    }
} else {
...
ShowFlags("ladder.php", "&mode=" . $mode . "&server=" . $_GET['server']);
...
foreach ($Statstable as $modeofstats => $unused) {
    $i++;
    echo "<a class=nav href=\"ladder.php?mode=" . $modeofstats . "&server=" . $_GET['server'] . "\">" . $text_gamemode[$modeofstats] . "</a>&nbsp;&nbsp;&nbsp;&nbsp;";
    if ($i == 4) {
        $i = 0;
        echo "<br>";
    }
...
$countedrows = db_query($db, $countrows);
$ranked = db_num_rows($countedrows);
if ($ranked > 0) {
    if (!isset($site)) {
        $site = 1;
    }
    ...
    for ($si = 1; $si <= $sitecount; $si++) {
        echo "<a class=nav href=\"ladder.php?mode=" . $mode . "&server=" . $_GET['server'] . "&site=" . $si . "\">-" . $si . "-</a> ";
    }
    echo "</td></tr>";
    ...
    $looked = db_query($db, $look);
    if ($serverident == "%") {
        $smode = "rand";
    } else {
        $smode = "randende";
    }
    while ($place = db_fetch_array_assoc($looked)) {
        $i++;
