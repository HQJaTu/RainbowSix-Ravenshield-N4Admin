...
if ($nicksearch != "") {
    ...
    $searchnick = db_query($db, "SELECT " . $Nicktable . ".nick as nick FROM " . $Nicktable . " LEFT JOIN " . $Playertable . " ON (" . $Nicktable . ".fromid = " . $Playertable . ".id) WHERE " . $Nicktable . ".nick like '" . $nicksearchencoded . "' and " . $Playertable . ".ubiname <> '' and " . $Nicktable . ".nick <>'' and " . $Playertable . ".serverident like '" . $serverident . "' ORDER BY nick");
    $nickcount = db_num_rows($searchnick);
...
foreach ($serveridents as $server => $serverident) {
    if (isset($Statstable[$mode])) {
        ...
        while ($ubinames = db_fetch_array_assoc($resubinames)) {
            ...
            $i++;
            $resnick = db_query($db, "SELECT * FROM $Nicktable WHERE fromid='$fromid' and nick <>''");
            $j = 0;
            while ($nicks = db_fetch_array_assoc($resnick)) {
                if ($j++ > 0) {
                    $ubiandnick .= ", ";
                }
            ...
            if (strstr($playersearch, "%")) {
                $scoreview = False;
                $nostatsshow = False;
                $resstats = db_query($db, "SELECT sum(kills) as sumkills, sum(deaths) as sumdeaths ,sum(roundsplayed) as sumrounds,sum(fired) as sumfired,sum(hits) as sumhits FROM $Statstable[$mode] WHERE fromid='$fromid'");
                ...
                $scoreview = True;
                $nostatsshow = True;
                $resstats = db_query($db, "SELECT * FROM $Statstable[$mode] WHERE fromid='$fromid' ORDER BY map");
                while ($stats = db_fetch_array_assoc($resstats)) {
...
?>
<?php print Copyrightextstats() ?></form></center></body>
